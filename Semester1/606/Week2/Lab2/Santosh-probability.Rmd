...
1.  What does a streak length of 1 mean, i.e. how many hits and misses are in a 
    ...
    Answer: A streak length of 1 says it was single hit event followed by a miss. 
    A streak length of 0 means it was miss.
...
2.  Describe the distribution of Kobe's streak lengths from the 2009 NBA finals. 
    ...
    Answer: The distribution of Kobe's streak length is unimodal and right skewed.
    His typical streak length was 0 and his longest streak of baskets was 4
...
-   Describe the distribution of streak lengths. What is the typical streak 
    ...
    The distribution of streak length is unimodel and right skewed as there are less high length streaks
...
```
   ...
   If you were to run the simulation of the independent shooter a second time, 
    ...
    different? Explain your reasoning.
     ...
     If I were to run the simulation of the independent(simulated) shooter again, I would expect somewhat similar distribution        results since the sample size is not small to drastically change the end result, and also the probability of .45 making it  result in more or less same results.
   ...
   How does Kobe Bryant's distribution of streak lengths compare to the 
    ...
    Comparing the Kobe Bryant's and Simulated shooter's distribution of streak length, it is visible that  there is no evidence of hot hand, since the number of success streaks is max at 1 and 2 is no where closer to it. Which means the probability of consecutive hits is independent.
