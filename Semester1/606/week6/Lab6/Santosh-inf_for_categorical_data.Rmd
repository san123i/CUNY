...
1.  In the first paragraph, several key findings are reported. Do these 
    ...
    Answer: These are sample statistics since it's not possible to measure entire world's resposne.
...
2.  The title of the report is "Global Index of Religiosity and Atheism". To
    ...
    Answer: To generalize the report's findings to the entire human population, we need to assume that it was randomly cluster sampled. 
...
5.  Write out the conditions for inference to construct a 95% confidence
    ...
    Answer:
    H0 - The proportion of atheists in the sample matches to the proportion of atheists in the world population 
    HA - The proportion of atheists in the sample doesn't match the proportion of atheists in the world population 
    
    Conditions:
    The population samples are independent - Pass
    There is a success-failure cases of atleast 10 each in the sample - Pass
...
11. If you refer to Table 6, you'll find that Australia has a sample 
  ...
  Therefore, as per success-failure case, we can with inference and report margin of errors for Australia but not Eucador.
...
inference(es_atheist_2012_sample, est = "proportion", type = "ci", method = "theoretical", success = "atheist")



p <- 0.1
es_atheist_2005_sample <- sample(c("atheist", "non_atheist"), n, replace = TRUE, prob = c(p, 1-p))
inference(es_atheist_2005_sample, est = "proportion", type = "ci", method = "theoretical", success = "atheist")
...
```
    ...
    **b.** Is there convincing evidence that the United States has seen a
    ...
    Answer: We cannot evaluate since there aren't atleast 10 success or failure case scenarios.
