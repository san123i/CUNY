...
1.  In the first paragraph, several key findings are reported. Do these 
    ...
    **Answer: These are sample statistics since it's not possible to measure entire world's response**
...
2.  The title of the report is "Global Index of Religiosity and Atheism". To
    ...
    **Answer: To generalize the report's findings to the entire human population, we need to assume that it was randomly cluster sampled, with each country representing a cluster **
...
3.  What does each row of Table 6 correspond to? What does each row of 
    ...
    **Answer: Each row of table 6 representations the observations from the sample taken from each country. **
    **The variable 'athesim' represents people who declared themselves as aethists in the sample. **
...
us12_a <- subset(us12, response=="atheist")
nrow(us12_a)/nrow(us12)

```
Answer: Yes, the data matches to atheism 5% as mentioned in the table.
...
5.  Write out the conditions for inference to construct a 95% confidence
    ...
    **Answer: </br>    H0 - The proportion of atheists in the sample matches to the proportion of atheists in the world population <br>     HA - The proportion of atheists in the sample doesn't match the proportion of atheists in the world population **</br>
    
    **Conditions: </br>    The population samples are independent - Pass </br>     There is a success-failure cases of atleast 10 each in the sample - Pass **
...
6.  Based on the R output, what is the margin of error for the estimate of the 
    ...
    **Answer: Margin of Error <- z * standard of error </br>**
    **Therefore margin of error = `r 1.96 * .0069`**
...
7.  Using the `inference` function, calculate confidence intervals for the 
    ...
    **Answer: </br>Margin of error for Canada = 1.96 * SE = `r 1.96*.009` </br> Margin of error for Iceland = 1.96 * SE = `r 1.96*.0103`**
```{r}
ca12 <- subset(atheism, nationality == "Canada" & year == "2012")
inference(ca12$response, est = "proportion", type = "ci", method = "theoretical", success = "atheist")
ic12 <- subset(atheism, nationality == "Iceland" & year == "2012")
inference(ic12$response, est = "proportion", type = "ci", method = "theoretical", success = "atheist")
```
...
8.  Describe the relationship between `p` and `me`.
  ...
  Answer: `p` and `me` both increase till `p` reaches .5 and after that `me` reduces as the `p` value increases. So `me` is the highest when `p` is at .5
...
9.  Describe the sampling distribution of sample proportions at $n = 1040$ and 
    ...
    **Answer: The distribution is almost normal with a mean of 0.01, sd of .009 and IQR of (.093 to .105)**
```{r}
summary(p_hats)
```
...
11. If you refer to Table 6, you'll find that Australia has a sample 
  ...
  Answer:
  About 0.1 or 10% of the sample in Australia comes to 104 people. And for Eucador, .02% makes just 8 people.
  Therefore, as per success-failure case, we can proceed with inference and report margin of errors for Australia but not Eucador.
...
-   Answer the following two questions using the `inference` function. As 
    ...
    *Hint:* Create a new data set for respondents from Spain. Form
    ...
    Answer: Writing up the hypothesis test
    H0 - There is no change in the atheism in Spain in 2005 and 2012
    HA - There is a change in the atheism in Spain in 2005 and 2012
    
```{r}
n <-  1146
p <- .09 
es_atheist_2012_sample <- subset(atheism, nationality=="Spain" & year=="2012")
es_atheist_2005_sample <- subset(atheism, nationality=="Spain" & year=="2005")

#es_atheist_2012_sample <- sample(c("atheist", "non_atheist"), n, replace = TRUE, prob = c(p, 1-p))
inference(es_atheist_2012_sample$response, est = "proportion", type = "ci", method = "theoretical", success = "atheist")
inference(es_atheist_2005_sample$response, est = "proportion", type = "ci", method = "theoretical", success = "atheist")
...
```
  ...
  **b.** Is there convincing evidence that the United States has seen a change in its atheism index between 2005 and 2012?
      Answer: Writing up the hypothesis test
        H0 - There is no change in the atheism in the United States in 2005 and 2012
        HA - There is a change in the atheism in the United States in 2005 and 2012
```{r}

us_atheist_2012_sample <- subset(atheism, nationality == "United States" & year == "2012")
us_atheist_2005_sample <-  subset(atheism, nationality == "United States" & year == "2005")
#es_atheist_2012_sample <- sample(c("atheist", "non_atheist"), n, replace = TRUE, prob = c(p, 1-p))
inference(us_atheist_2012_sample$response, est = "proportion", type = "ci", method = "theoretical", success = "atheist")
inference(us_atheist_2005_sample$response, est = "proportion", type = "ci", method = "theoretical", success = "atheist")
```
  
    
    Answer: Since the confidence intervals match, we can assume that H0 null hypothesis is true, and there is no change in atheism index in the United States b/w 2005 and 2012.
...
-   Suppose you're hired by the local government to estimate the proportion of 
    ...
    *Hint:* Refer to your plot of the relationship between $p$ and margin of 
    ...
    Answer: Since we are not provided with a p value. We will assume p as 0.5 since margin of error is max as 0.5
