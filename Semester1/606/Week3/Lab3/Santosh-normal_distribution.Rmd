...
1.  Make a histogram of men's heights and a histogram of women's heights.  How 
    ...
    **<u>Answer:</u>** Males histogram of heights represents a unimodal normal distribution. Females histogram of heights represents a multimodal normal distribution
```{r}
ggplot(data = mdims, aes(mdims$hgt)) + geom_histogram()


ggplot(data = fdims, aes(fdims$hgt)) + geom_histogram()
```
...
2.  Based on the this plot, does it appear that the data follow a nearly normal 
     ...
     **<u>Answer:</u>** Yes, upon evaluating the plot, it appears that the data follows a normal distribution pattern.
...
3.  Make a normal probability plot of `sim_norm`.  Do all of the points fall on 
    ...
    **<u>Answer:</u>** On below plot almost all the points fall on the line excluding the tail ends. This plot is much more normally distributed compared to the real data set.
```{r}
qqnorm(sim_norm)
qqline(sim_norm)
...
4.  Does the normal probability plot for `fdims$hgt` look similar to the plots 
    ...
    female heights are nearly normal? </br>
    
    **<u>Answer:</u>** Yes, the probability plot for 'fdims$hgt' seem to be near normal.

5.  Using the same technique, determine whether or not female weights appear to 
    come from a normal distribution.
    
    Answer: The below plot shows that most of the data points lie on normal plot line, but there are significant number of points which are out of line at the tail ends.
    There are few outliers too.
```{r, eval=TRUE}
...
6.  Write out two probability questions that you would like to answer; one 
    ...
    **What is the probability that a randomly chosen young  adult female is shorter than 6 feet **
```{r}
...
```
    ...
    **What is the probability that a randomly chosen young adult female weights more than 60 lb**
```{r}
...
-   Now let's consider some of the other variables in the body dimensions data 
    ...
    **a.** The histogram for female biiliac (pelvic) diameter (`bii.di`) belongs
    to normal probability plot letter **B**. </br>
    **b.** The histogram for female elbow diameter (`elb.di`) belongs to normal 
    probability plot letter **C**.</br>
...
-   Note that normal probability plots C and D have a slight stepwise pattern.  
    ...
    **Answer:** I think it could be due to high number of similar values 
...
-   As you can see, normal probability plots can be used both to assess 
    ...
    **Answer:** Below plot shows that the plot for female knee diameter is 'Right skewed'
```{r}
qqnorm(fdims$kne.di)
qqline(fdims$kne.di)

hist(x = fdims$kne.di, probability = T)
```
