...
1.  How many cases are there in this data set?  How many variables?  For each 
    variable, identify its data type (e.g. categorical, discrete).</br>
### There are 20000 cases, and 9 variables. 
    ...
    * `height` - Discrete Numeric(per given dataset)
    * `weight` - Discrete Numeric(per given dataset) 
    * `wtdesire` - Discrete Numeric(per given dataset) 
...
3.  What does the mosaic plot reveal about smoking habits and gender?</br>
###The mosaic plot reveals that proportion of smoking population is higher in males than females
</br>
...
4.  What does this box plot show? Pick another categorical variable from the 
    ...
    Answer: The box plot shows that people with poor health having higher BMI's, and people with excellent health 
    I chose exerany (categorical) variable, since logically people who exercise maintain good health. The below bar plot suggests that average BMI's in people exercising are in proper range compared to others 

```{r boxplot-bmi-excercise, eval=TRUE}
bmi <- (cdc$weight / cdc$height^2) * 703
...
###The scatter plot shows below observations
###1) On average, there is almost a linear relation b/w weight and desired weight till 200 lbs, which means people under 200lbs are more or less closer to their desired weight
###2) On average, most of people above 200 lbs are higher than their desired weights
###3) The number of people who desire more than 300 lbs is just handful compared to the count who weigh more than 300
...
### If wdiff is 0 for a person's weight is as per his/her desired weight. If wdiff is more than 0 then the person needs to gain weight. If wdiff is less than 0 then the  person needs to loose weight
```{r}
typeof(wdiff)
...
### Below numbers and boxplot reveals that IQR range of the wdiff of the population is within -21 to 0. And there are outliers which fall out of min and max which as per the IQR is close to +/- 50. This indicates that a majority of the people feel they are well within the in a deviation of 50lbs from their desired weight. 
```{r}
summary(wdiff)
boxplot(wdiff)
```
...
###Below stats and boxplot suggest that IQR for men (20) is lower than women(27). Which means men view their weights are more closer to their desired weight when compared to women.
```{r}
