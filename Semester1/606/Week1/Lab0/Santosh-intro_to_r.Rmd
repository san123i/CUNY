...
2.  Is there an apparent trend in the number of girls baptized over the years?  
    How would you describe it? Answer: There was decline in number of baptized girls from 1640 to 1660, but rose again from 1660 hitting a max around 1700's
...
We cannot compare between `Arbuthnot` data and `Present` data since the data period doesn't match. However, the count of baptized boys and girls significantly lower when compared to the count in Present data.
...
`Arbuthnot` and `Present` data set both present an observation where boys ratio is slightly higher than girls. However, there is clear visible downtrend in boys ratio in `Present` data
``` {r}
plot(present$year, present$boys/present$girls, type = "l", main  = "Boys to Girls ratio (Present)", xlab = "Years", ylab = "Boys")

plot(arbuthnot$year, arbuthnot$boys/arbuthnot$girls, type = "l", main  = "Boys to Girls ratio (Arbuthnot)", xlab = "Years", ylab = "Boys")
```
