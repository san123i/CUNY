...
1.  What command would you use to extract just the counts of girls baptized? Try
    it! </br></br> **Answer: arbuthnot$girls**
...
2.  Is there an apparent trend in the number of girls baptized over the years?  
    How would you describe it? </br></br>
    **Answer: There was decline in number of baptized girls from 1640 to 1660, but rose again from 1660 hitting a max around 1700's**
...
the future.
```{r}
plot(x=arbuthnot$year, y=arbuthnot$boys / (arbuthnot$boys + arbuthnot$girls), ylab = "Boys proportion", type="l")
```
...
We cannot compare between `Arbuthnot` data and `Present` data since the data time period (16th vs 19th century) doesn't match. However, the count of baptized boys and girls is significantly lower when compared to the count in Present data.
...
**`Arbuthnot` and `Present` data set both present an observation where boys ratio is slightly higher than girls. However, there is a clear visible downtrend in boys ratio in `Present` data**
``` {r}
plot(present$year, present$boys/present$girls, type = "l", main  = "Boys to Girls ratio (Present)", xlab = "Years", ylab = "Boys/Girls")

plot(arbuthnot$year, arbuthnot$boys/arbuthnot$girls, type = "l", main  = "Boys to Girls ratio (Arbuthnot)", xlab = "Years", ylab = "Boys/Girls")
```

### **Q4. In what year did we see the most total number of births in the U.S.? You can refer to the help files or the R reference card [http://cran.r-project.org/doc/contrib/Short-refcard.pdf](http://cran.r-project.org/doc/contrib/Short-refcard.pdf) to find helpful commands.**

```{r}
present$total_birth <- present$boys + present$girls
max_birth_year <- present[which.max(present$total_birth),]$year
max_birth_year
```
