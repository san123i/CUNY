...
1.  What type of plot would you use to display the relationship between `runs` 
  ...
  **Answer: A scatter plot will be a better fit in this scenario to display the relationship b/w runs and other variables.  The relation b/w `runs` and `at_bats` is not fully linear, but is close to a linear model.**
```{r}
...
2.  Looking at your plot from the previous exercise, describe the relationship 
  ...
  **Answer: The strength of the relationship can be derived from the `cor` function which is .61. And the direction appears to be positive and near linear. **
  **Below is the form, with an intercept of -2789.2429 and slope of .6305**
...
3.  Using `plot_ss`, choose a line that does a good job of minimizing the sum of
  ...
  **Answer: After running several times the function, the smallest sum of squares I have seen is 141575.**
...
4.  Fit a new model that uses `homeruns` to predict `runs`. Using the estimates 
  ...
  **Answer:**
  **The slope is more for homeruns compared to atbats. That means the change in number of `runs` is high with change in `homeruns`. Therefore, high home runs means high success ratio when compared to `atbats`**

\[
  \hat{y} = 415.234 + 1.83 * homeruns
\]
...
5.  If a team manager saw the least squares regression line and not the actual 
   ...
   ** Answer: If a person looks at the least squares regression line, then for a at-bats of 5,578 and estimate of around 710 will be made. But this is an overestimate since all the actual values around 5578 are under 710.**
...
6.  Is there any apparent pattern in the residuals plot? What does this indicate
  ...
  **Answer: The number of points seems to follow a near normal distribution with right skew, but if we look at the value of residuals then we cannot identify any pattern.**
...
7.  Based on the histogram and the normal probability plot, does the nearly 
   ...
   **Answer: Yes, based on the historgram and normal plot, we can see that normal residual condition seems to be met since there is only a slight skew.**
...
**Answer: Yes, based on the plot the constant variability condition appears to be met since the variability seems to be constant.**
...
**Answer: I choose `hits` variable to be another predictor of `runs`. Looking at the scatter plot, we can see a direct linear relation b/w `runs` and `hits`.**
...
**Answer: From the below summary we can see that strength is 80.12% and the R$^2$ value when using `hits` is 64.2% which is better than `at_bats` which is at 63.05%. Yes, since the strength is higher and R$^2$ values are better than `at_bats` value, `hits` is a better of `runs`.**
```{r}
summary(m2)
...
**Answer: After comparing all remaining traditional variables (homeruns, bat_avg, strikeouts, stolen_bases, wins), I noticed that bat_avg is best at predicting the runs amongst  these 5 variables. Looking at the below given graphical and numerical evidences, the strength is higher for `bat_avg`.**
...
**Answer: Of all the ten variables ( 7 traditional and 3 new variables), the new variables are better predictors of `runs`. Of the 3 new variables, `new_obs` is the most accurate predictor with a correlation strength of 97%.**
...
```
  ...
  5.   Check the model diagnostics for the regression model with the variable you 
    ...
    Linearity:
```{r}
mbnewObs <- lm(runs ~ new_obs, data = mlb11)
...
plot(mbnewObs$residuals ~ mlb11$new_obs)
abline(h = 0, lty = 3)

hist(mbnewObs$residuals)
qqnorm(mbnewObs$residuals)
qqline(mbnewObs$residuals)
```
