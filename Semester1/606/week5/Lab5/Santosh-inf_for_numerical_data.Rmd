...
**Answer.  These cases are the birth records in North Carolina in 2004. The number of cases in our sample is `r nrow(nc)`  **
...
**Answer: The plot b/w habit and weight describes that median weight of the baby is slightly higher in case of non-smoking mothers.**
...
**Answer: The sample size of 1000 is large enough and the observations are independent of each other. Also, below distribution of `weight` data shows that the data distribution is normal.**
```{r}
ggplot(nc, aes(x = weight)) + geom_bar()
```
...
**Answer: The null hypothesis (H0) states that avg weights of babies born to smoking and non-smoking mothers are same. And the alternate hypothesis(HA) states that the avg weights are different for different habit mothers.**
...
**Answer:  From the below inference function results, we can clearly state with a 95% confidence that avg length of pregnancies (in weeks) is between 38.15 and 38.5 weeks.  **
```{r}
...
**Answer: Let's make a null hypothesis (H0) that avg weight gained by young mothers and mature mother are same i.e., there is no difference. And an Alternate Hypothesis (HA) that avg weight gained by young and mature mothers are different.**
```{r}
...
```{r}
youngMom <- subset(nc, mature="younger mom")
youngMin <- min(youngMom$fage, na.rm = T)
youngMax <- max(youngMom$fage, na.rm = T)


matureMom <- subset(nc, mature="mature mom")
matureMin <- min(matureMom$fage, na.rm = T)
matureMax <- max(matureMom$fage, na.rm = T)
```
**Answer: The cut off age for younger moms is `r youngMin` and `r youngMax` </br> The cut off age for mature moms is `r matureMin` and `r matureMax`**
...
Answer: Evaluate the relationship b/w baby gender and the baby weight. 
Applying Hypothesis testing, 
Null Hypothesis (H0) -   The avg weight of female and male babies are same.
Alternate Hypothesis (HA) - The avg weight of female and male babies are different

The below inference method results show that Alternate hypothesis is true since the value 0 doesn't fall under confidence intervals.
```{r}
