...
1.  Describe this population distribution. <br>
   
**Answer: This distribution is unimodal and right skewed**
...
**Answer:The distribution of the sample is unimodal, with a right skew**
```{r}
samp1 <- sample(area, 50)
```
...
**Answer: The mean of samp2 is different compared to samp1, since both sample sizes are relatively smaller(50). If we pick a higher sample size (of 100 or 1000) then the mean values will be much closer to the actual mean of the population**
```{r}
...
**Answer: There are 5000 elements in sample_means50. The sampling distribution of sample_means50 is normal and symmetric in nature. Since the sampling of 50000 is higher than 5000 I expected an even more symmetric distribution chart, but in overall the distribution should be close to earlier one(5000)**
...
**Answer: There are 100 elements in sample_means_small. Each element represents a mean of each sample(of 50)**
```{r}
sample_means_small <-  rep(NA, 5000)
for(i in 1:100) {
...
**Answer: When the sample size gets larger the spread decreases since most of the elements gets closer to the center, but center more or less remains unchanged.**
...
priceSamp50 <- sample(prices, 50)
poeMean <- mean(priceSamp50)
poeMean

```
Answer: Point of estimate for population mean using a sample size of 50 is `r poeMean`
...
sample_means50 <- rep(NA, 5000)
for(i in 1:5000) {
  samp <- sample(prices, 50)
  sample_means50[i] <- mean(samp)
}
hist(sample_means50)
#Below is the mean of samples(5000) collected
mean(sample_means50)
```

Below is the mean of total population
```{r}
mean(prices)
deviation50 <- round((((mean(prices)-mean(sample_means50)) / mean(prices)) * 100), 2)
```

Deviation percentage of sample mean(50) from population mean is very minor `r deviation50`%
...
```{r}
deviationSample150 <- round(((mean(prices)-mean(sample_means150)) / mean(prices)) * 100, 2)
deviationSample150
```

Deviation percentage of sample mean(150) from population mean is very minor **`r deviationSample150`%**
...
The sample with 150 values i..e, Q3 has a smaller spread. I believe we should be focussing on having a sample which can closely represent the actual population. Therefore, since a larger sample results in smaller spread, we should prefer having a small spread.
