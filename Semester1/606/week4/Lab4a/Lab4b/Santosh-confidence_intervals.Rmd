...
**Answer: The sample being a random sample can give you different values each time you pull a sample. Bu the sample generated previously was a unimodal with  a right skew. The typical value of a sample can be denoted as a mean value which in this case is `r meanValue`**
```{r}
hist(samp)
```
...
**Answer: It is a possibility for another student's distribution to be similar to mine. As previously mentioned the sample are picked randomly, so there may be another student who may have similar values to mine, and also none may get values similar to mine.**
...
**Answer:**
  1. The sample should be larger than 30 and less than 10% of total population
  2. The sample should not be extremely skewed
  3. The events should be independent of each other.

## Confidence levels

4.  What does "95% confidence" mean? If you're not sure, see Section 4.2.2.

**Answer: A 95% confidence means that there is a 95% chance the sample means picked will conain an actual mean value.**
...
upper <- as.numeric(mean(samp) + 1.96*sampleSE)
lower <- as.numeric(mean(samp) - 1.96*sampleSE)
popMean <- mean(population)
if(popMean >lower && popMean<upper) {
  ("Mean falls under 95% Confidence Interval")
} else { 
  ("Mean doesn't fall under 95% Confidence Interval")
}
```
...
population mean.
**Answer: These are independent events and cannot be guaged. But going by CI, there is a 95% probability of those students values capture actual mean.**
...
Answer: Selecting a 99% CI
```{r}
cl <- 99
alp <- 1 - cl/100
value <- 1 - alp/2 #Each tail 1/2
cv <- qnorm(value)
cv
```
...
Answer: I see a match with 99% CI matching the value provided.
```{r}
