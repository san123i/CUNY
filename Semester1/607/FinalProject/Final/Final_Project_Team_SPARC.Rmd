...
train <- read_tsv("Datasets/drugLibTrain_raw.tsv", col_names = TRUE, cols(
...
test <- read_tsv("Datasets/drugLibTest_raw.tsv", col_names = TRUE, cols(
  X1 = col_double(),  urlDrugName = col_character(),  rating = col_double(),  effectiveness = col_factor(),  sideEffects = col_factor(),  condition = col_factor(),
  benefitsReview = col_character(),  sideEffectsReview = col_character(),  commentsReview = col_character()
))
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
review_summary %>% 
  mutate(quadrant = case_when(mean_rating > x_mid & sentiment > 0 ~ "Positive-Postive", mean_rating <= x_mid & sentiment > 0 ~ "Negative-Positive", mean_rating <= x_mid & sentiment <= 0 ~ "Negative-Negative", TRUE ~ "Positive-Negative")) %>% ggplot(aes(x = mean_rating, y = sentiment, color = quadrant)) + geom_hline(yintercept=0, color = "black", size = 0.5) + geom_vline(xintercept = x_mid, color = "black", size = 0.5) + guides(color = FALSE) + scale_color_manual(values = c("blue", "red", "red","blue")) + ggtitle("Rating vs Sentiment Rating of Effectiveness") + geom_point()
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
review_summary %>% 
  mutate(quadrant = case_when(mean_rating > x_mid & sentiment > 0 ~ "Positive-Postive", mean_rating <= x_mid & sentiment > 0 ~ "Negative-Positive", mean_rating <= x_mid & sentiment <= 0 ~ "Negative-Negative", TRUE ~ "Positive-Negative")) %>% ggplot(aes(x = mean_rating, y = sentiment, color = quadrant)) + geom_hline(yintercept=0, color = "black", size = 0.5) + geom_vline(xintercept = x_mid, color = "black", size = 0.5) + guides(color = FALSE) + scale_color_manual(values = c("blue", "red", "red","blue")) + ggtitle("Rating vs Sentiment Rating of Side Effect") + geom_point()
```
